...
const JWT_SECRET = process.env.JWT_SECRET || String(process.env.SUPABASE_SECRET_KEY || 'doom_jwt_secret_must_be_set_in_env');
...
export async function loginOrSignupWithGoogle({ email, name = '' }) {
  const cleanEmail = String(email || '').trim().toLowerCase();
  if (!cleanEmail || !cleanEmail.includes('@')) throw new Error('Valid Google email required');

  const now = Date.now();

  if (supabase) {
    let { data: user } = await supabase.from('users').select('*').eq('email', cleanEmail).single();

    if (!user) {
      const { data: newUser, error } = await supabase.from('users').insert({
        email: cleanEmail,
        password_hash: 'google_oauth_' + crypto.randomBytes(8).toString('hex'),
        name: name || cleanEmail.split('@')[0],
        plan: 'free',
        credits: 50,
        validity_expires_at: new Date(now + VALIDITY_MS).toISOString(),
  ...
  if (!user) {
    user = {
      id: 'usr_' + crypto.randomBytes(8).toString('hex'),
      email: cleanEmail,
      passwordHash: 'google_oauth_' + crypto.randomBytes(8).toString('hex'),
      name: name || cleanEmail.split('@')[0],
      plan: 'free',
      credits: 50,
      validityExpiresAt: now + VALIDITY_MS,
